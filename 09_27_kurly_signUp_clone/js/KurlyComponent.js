...
class KurlyComponent extends React.Component {
  ...
  constructor(props){
    ...
    this.state = {
      ...
      추가입력사항: '', //라디오버튼
      isAddrInputBoxShow: false, //추가입력박스
      addInputPlaceHolder:'',
      추가입력상자: '', //추가입력상자 입력값

      이용약관동의: [], //체크박스 다중선택 저장 -> 객체 배열
      isAddressOn: false, //주소 검색을 클릭하면 true로 변환 그럼 주소검색창이 열린다.
      isAddrInputShow: false,

      showId: false, //아이디의 가이드 텍스트 show
      isClassId: '', //클래스를 '' : true : false(보여주고 안보여주는 역할)

      showPw: false, //비밀번호의 가이드 텍스트 show
      isClassPw1: '', //클래스를 '' : true : false(보여주고 안보여주는 역할)
      isClassPw2: '',
      isClassPw3: '',
      ...
      // showYear: false, //생년 가이드텍스트 show
      // showMonth: false, //생월 가이드텍스트 show
      // showDay: false, //생일 가이드텍스트 show
      // isClassYear: '', //생년 클래스를 '' : true : false
      // isClassMonth: '', //생월 클래스를 '' : true : false
      // isClassDay: '', //생일 클래스를 '' : true : false
  ...
  onChangeId=(value)=>{
    ...
    this.inputIdtext= value; //생성자때문에 전역변수
  ...
  //아이디중복확인 반복처리문
  //1. 아이디에 입려값 === 저장된 아이디와 비교
  //2. 로컬스토레이지(데이터베이스)데이터 전체 추출
  //3. 추출한 데이터에서 아이디만 추출
  //4. 반복처리 비교하고 같은 아이디가 있다면 true로 변환
  //5. 결과가 true이면 등록된 아이디입니다
  //6. 결과가 false이면 사용가능한 아이디입니다
  ...
  onClickIdModalEvent=(e)=>{
    ...
    if(this.state.아이디===''){
      ...
      if(this.state.isClassId===false){ //아이디 정규표현식이 결과가 false인 경우
        this.setState({
        ...
        // 같은 아이디가 있으면 true 발생(중복)
        // 같은 아이디가 없으면 false 발생(중복x)
...
//가이드 텍스트 show
  ...
  onChangeName=(value)=>{
    ...
    // .test() true & false
    // 문자열.replace(정규표현식,'') 정규표현식이 true 이면 변경('') 공백으로 삭제 
  ...
  //중복확인 아이디 클릭 모달 이벤트
    onClickEmailModalEvent=(e)=>{
      ...
      if(this.emailValue===''){
        ...
        if(regExp.test(this.emailValue)===false){ //이메일 정규표현식이 결과가 false인 경우
          this.setState({
  ...
  onChangePhone=(value)=>{
    ...
    //휴대폰 번호가 10자리 이상이면 우측에 인증번호받기 버튼이 보인다.
    if(value.length >= 10){
    ...
    //휴대폰 번호 정규표현식
    //입력이 완료되고 난 후 인증번호 버튼을 클릭해서 실행할 때 사용
  }
  ...
  onClickPhoneEvent=(e)=>{
    e.preventDefault();
    //휴대폰 번호 입력상자가 반값이면 버튼은 클릭이 취소된다.
    if(this.phoneValue!==undefined){
      if(this.phoneValue.length<10){
        return;
      }
      else{
        if(!/^01[016789]{1}[0-9]{3,4}[0-9]{4}$/g.test(this.state.휴대폰)){ //not true = false
  
          this.setState({
            휴대폰ok: false,
            isModalOpen: false, //모달 열기
  ...
  onClickPhoneOkEvent=(e)=>{
    ...
    if(this.state.휴대폰인증===this.state.인증키){
      this.setState({
        isModalOpen: true, //모달 열기
        modalText: '인증이 완료되었습니다.', //가이드 텍스트
        isTimerOpen: false, //카운터 타이머 숨기기
        disabled2: true,
        isPhoneOkClass: false,
        휴대폰: this.state.휴대폰.replace(/^(01[016789]{1})([0-9]{3,4})([0-9]{4})$/g, '$1-$2-$3'), 

      });
      clearInterval(this.state.setId2);
    }
    else{
      this.setState({
        isModalOpen: true, //모달 열기
        modalText: '인증번호를 확인하세요.', //가이드 텍스트
        isTimerOpen: false, //카운터 타이머 숨기기
        disabled2: true
      });
    }
  ...
  counterTimer(){
    //반드시 화살표 함수 사용
    let setId2 = setInterval(()=>{ //set은 환경변수
    ...
    this.setState({setId: setId2}) //전역변수 등록
  }
  //컴포넌트 디드 마운트
  //렌더링 이후에 동작하는 메서드
  componentDidMount(){
  ...
  onChangeAddress1=(zip, value)=>{
    ...
    this.setState({inAddressOn: false}); //검색이 완료되면 다시 초기화
    this.setState({isAddrInputShow: true}); //재검색버튼이 보이고 입력상자도 우편번호, 주소1, 주소2 모두 보인다.
  }
  ...
  onChangeGender=(value)=>{
    ...
    console.log(this.state.성별.includes(`남자`));
    console.log(this.state.성별.includes(`여자`));
    console.log(this.state.성별.includes(`선택안함`));
  }
  ...
  /////////////////////////////////////////////////////
    ...
    // 1900 - 2000
    // 1999 - 2999
    // const regExp1 = /(?:1[0-9][0-9][0-9]|2[0-9][0-9][0-9])/g; //?: -> 삼항연산자
    // const regExp = /^(?:1\d\d\d|2\d\d\d)$/g; //?: -> 삼항연산자
    // console.log(regExp.test(value));

    //2. 생월
    //1-12
    //const regExp = /^(?:0?[1-9]|1[012])$/g; //1-9라는 범위가 있어서 \d는 불가
    //console.log(regExp.test(value));
  ...
  //4. 단 년, 월, 일 모두 입력제한조건을 만족한 경우에 아래의
  //3가지 조건을 마지막 조건으로 다시 구현한다.
  //4-1. 미래의 생년월일 가입불가
  //4-2. 만 14세미만 가입불가
  //4-3. 만 100세 초과 가입불가 
  ...
  birthDayCheckEventFn=(z)=>{
    ...
    const lastDay = new Date(생년, 생월, 0).getDate(); //0->말일, 1->다음달 첫날
    ...
    const nowToday = new Date(nowYear, nowMonth, nowDay); //현재년월일 변수
    const nowToday14 = new Date(nowYear-14, nowMonth, nowDay); //현재년월일 변수
    const nowToday100 = new Date(nowYear-100, nowMonth, nowDay); //현재년월일 변수
      ...
      if(/(?:1[9][0-9][0-9]|2[0-9][0-9][0-9])/g.test(생년)===false){
        ...
        //가이드텍스트 show
         ...
         return;
          ...
          //가이드텍스트 hide
          //생일
          if(/^(?:0?[1-9]|1[0-9]|2[0-9]|3[0-1])$/g.test(생일)===false || 생일 > lastDay){ //false
            //가이드텍스트 show
            this.setState({
              showBirthDay: true, //show true, hide false
              isClassBirthDay: false, //'',true green,false red
            ...
            if(birthDay > nowToday){ //미래
                  ...
                  this.setState({
                    showBirthDay: true, //show true, hide false
                    isClassBirthDay: false, //'',true green,false red
                    birthDayGuideText: '생년월일을 미래로 입력되었어요.', //가이드텍스트 여러가지 내용  
                  })
                  return;
                }
                else{
  ...
  //포커스 아웃하면 발생되는 이벤트함수
  //순수자바스크립트에서
  //onFocus / onFocusIn => 리액트에서 사용 불가: 그래서 onFocus를 사용
  //onBlur / onFocusOut => 리액트에서 사용 불가: 그래서 onBlur를 사용
  //입력상자를 떠나면 
  onBlurEvent=(z)=>{
  ...
  //체크상태가 아니면(false)이면 약관동의 배열안에 배열값만 제외(삭제)한다.
  onChangeCheckEvent=(checked, value)=>{
    ...
    if(checked){ //true
      ...
      //조건문: SNS/이메일
      if(value==='SNS' && this.state.이용약관동의.includes('이메일')){
        this.setState({이용약관동의: [...this.state.이용약관동의, 'SNS', '무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)']})
      }
      // else if(value==='이메일' && this.state.이용약관동의.includes('SNS')){
      //   this.setState({이용약관동의: [...this.state.이용약관동의, '이메일', '무료배송, 할인쿠폰 등 혜택/정보 수신 동의']})
      // }
      // else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' && this.state.이용약관동의.includes('SNS') && !this.state.이용약관동의.includes('이메일')){
      //   this.setState({이용약관동의: [...this.state.이용약관동의, '이메일', value]})
      // }
      // else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' &&  this.state.약관동의.includes('이메일')  &&  !this.state.약관동의.includes('SNS') ){
      //   this.setState({약관동의: [...this.state.이용약관동의, 'SNS', value ]})
      // }
      // else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' && this.state.이용약관동의.includes('이메일') && this.state.이용약관동의.includes('SNS')){
      //   this.setState({이용약관동의: [...this.state.이용약관동의, value]})
      // }
      // else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' && !this.state.이용약관동의.includes('이메일') && !this.state.이용약관동의.includes('SNS')){
      //   this.setState({이용약관동의: [...this.state.이용약관동의,'이메일', 'SNS', '무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)']})
      // }
      // //
      else if(value==='이메일' &&  this.state.이용약관동의.includes('SNS') ){
        this.setState({이용약관동의: [...this.state.이용약관동의, '이메일', '무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)']})
      }
      else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' &&  this.state.이용약관동의.includes('SNS')  &&  !this.state.이용약관동의.includes('이메일') ){
        this.setState({이용약관동의: [...this.state.이용약관동의, '이메일', value ]})
      }
      else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' &&  this.state.이용약관동의.includes('이메일')  &&  !this.state.이용약관동의.includes('SNS') ){
        this.setState({이용약관동의: [...this.state.이용약관동의, 'SNS', value ]})
      }
      else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' &&  this.state.이용약관동의.includes('이메일')  &&  this.state.이용약관동의.includes('SNS') ){
        this.setState({이용약관동의: [...this.state.이용약관동의, value ]})
      }
      else if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)' &&  !this.state.이용약관동의.includes('이메일')  &&  !this.state.이용약관동의.includes('SNS') ){
        this.setState({이용약관동의: [...this.state.이용약관동의, '이메일', 'SNS',  value ]})
      }

      else{
        this.setState({이용약관동의: [...this.state.이용약관동의,value]}); //배열에 데이터 추가하기(누적하기)  ES5 취미.push(value)≈
      }

    }
    else{ //false 삭제(제외)
      if(value==='무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)'){
          result = this.state.이용약관동의.filter((item)=>item!==value);
        ...
        //let result = this.state.약관동의.filter(item=>item!==value); //(item) 괄호 생략 가능, 매개변수 1개인 경우
        result = this.state.이용약관동의.filter((item)=>item!==value);
      } //중괄호를 사용한 경우는 반드시 리턴문 사용해야한다.      //return없이도 가능-> 소괄호만 사용 시
      this.setState({이용약관동의: result});
    }
  }
  ...
  onChangeCheckEventAll=(checked, value)=>{
    ...
    if(checked){
      this.setState({이용약관동의: imsi}); //전체체크내용저장
    }
    else{
      this.setState({이용약관동의: [] }) //모두삭제
    }
  }

  //1. 아이디부터 이용약관까지 모두 검증하고
  //2. 검증된 데이터만 전송합니다. 저장
  ...
  onSubmitEvent=(e)=>{
    e.preventDefault();
    const {
           아이디,비밀번호,이름,이메일,휴대폰,우편번호,주소,주소1,주소2,성별,생년,생월,생일,추가입력사항,추가입력상자,이용약관동의,
           아이디ok,비밀번호1ok,비밀번호2ok,비밀번호3ok,이메일ok,휴대폰ok,아이디중복확인버튼
    } = this.state;

    if(아이디===''|| 비밀번호===''|| 이름===''|| 이메일===''|| 휴대폰===''|| 주소1==='' || 주소2===''){
      if(아이디===''){   
        this.setState({
          isModalOpen: true, //모달창 show & hide
          modalText: '아이디를 입력해주세요.',  //아이디/이메일/인증번호받기 가이드 텍스트  
        })   
      }
      else if(비밀번호===''){   
        this.setState({
          isModalOpen: true, //모달창 show & hide
          modalText: '비밀번호를 입력해주세요.',  //아이디/이메일/인증번호받기 가이드 텍스트  
        })
      }
      else if(이름===''){   
        this.setState({
      ...
      console.log( '아이디중복확인버튼' , 아이디중복확인버튼  );

      //입력값 규칙을 어기면 입력취소
      if(아이디ok===false || 비밀번호1ok===false || 비밀번호2ok===false || 비밀번호3ok===false || 이메일ok===false || 휴대폰ok===false || 아이디중복확인버튼===false){
        if(아이디ok===false){
          this.setState({
            isModalOpen: true,
            modalText: '아이디는 6자 이상의 영문 혹은 영문과 숫자 조합만 가능합니다.'
        ...
        let cnt=0;
        //약관동의 필수 3개 체크해야 전송
        for(let i=0; i<이용약관동의.length; i++){
          if(이용약관동의[i].indexOf('필수') >= 0){ //찾았다면 못찾으면 -1
            cnt++;
          }
        }
            ...
            <ul>
                ...
                <div className='gap'>
                  ...
                  <div className='input-box'>
                    ...
                    {   
                     this.state.showId && (
                      <div>
                        <p className={ //클래스 추 3항연산자
                            (
                              this.state.isClassId===''?'':(this.state.isClassId===true?'green':'red') //정규표현식약자:RefExp
                            )
                    ...
                    /> 
                    {
                      this.state.showPw && (
                        <div>
                            <p className={
                              (
                                this.state.isClassPw1===''?'':(this.state.isClassPw1===true?'green':'red') //정규표현식약자:RefExp
                              )
                            }
                            >10자 이상 입력</p>
                            <p className={
                              (          
                                this.state.isClassPw2===''?'':(this.state.isClassPw2===true?'green':'red') //정규표현식약자:RefExp
                              )
                            }>영문/숫자/특수문자(공백 제외)만 허용하며, 2개 이상 조합</p>
                            <p className={
                              (          
                                this.state.isClassPw3===''?'':(this.state.isClassPw3===true?'green':'red') //정규표현식약자:RefExp
                              )
              ...
              <li>
                <div className='gap'>
                  ...
                  <div className='input-box'>
                    ...
                    className='inputText'
                    id='inputEmail'
                    placeholder='예: marketkulry@kurly.com'
                        ...
                        <div>
                          ...
                          <button disabled={this.disabled2} onClick={this.onClickPhoneOkEvent} className={`w120-btn phoneok-btn ${this.state.isPhoneOkClass? ``:`phone`}`}>인증번호 확인</button>
                          {
                            this.state.isTimerOpen && (
                              <span className='conter-timer-box'>{this.state.minutes}:{this.state.seconds<10?`0${this.state.seconds}`:this.state.seconds}</span>
                            )
                        }
                          </div>
                      )
                ...
                <div className='gap'>
                  ...
                  <div className='input-box'>
                    ...
                    id='inputAddress'>
                      ...
                      <span>
                      {
                      this.state.isAddrInputShow===true? `주소 재검색`:`주소검색`
                      }
                      </span>
                    </button>
                ...
                <div className='gap'>
                  ...
                  <div className='input-box radio-box'>
                    ...
                    <label>
                      ...
                      type='radio'
                      onChange={(e)=>this.onChangeGender(e.target.value)}
                      checked={this.state.성별.includes(`여자`)?true:false}
                      value={`여자`}
                      />
                      <span>여자</span>
                    </label>
              ...
              <li>
                <div className='gap'>
                  ...
                  <div className='input-box'>
                      <ul className='date-box on' tabIndex='0'>
                        <li>
                          <input
                          type='text'
                          maxLength='4'
                        ...
                        <dl>
                          <dt>
                            <label>
                              ...
                              checked={this.state.이용약관동의.length >=7 ? true:false}
                              value={`전체 동의합니다.`}
                              />
                              <span>전체 동의합니다.</span>
                            </label>
                          ...
                          <dd>
                            <label>
                                ...
                                checked={this.state.이용약관동의.includes(`이용약관동의 (필수)`)}
                                value={`이용약관동의 (필수)`}
                                />
                                <span>이용약관동의 (필수)</span>
                            </label>
                          ...
                          <dd>
                            <label>
                                ...
                                id='chk2'
                                onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                checked={this.state.이용약관동의.includes(`개인정보 수집·이용 동의 (필수)`)}
                          ...
                          <dd>
                            <label>
                                ...
                                id='chk3'
                                onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                checked={this.state.이용약관동의.includes(`개인정보 수집·이용 동의 (선택)`)}
                          ...
                          <dd>
                            <label>
                                ...
                                id='chk4'
                                onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                checked={this.state.이용약관동의.includes(`무료배송, 할인쿠폰 등 혜택/정보 수신 동의 (선택)`)}
                            ...
                            <div className='sub-checkbox'>
                              <div>
                                <label>
                                  ...
                                  id='chk5'
                                  onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                  checked={this.state.이용약관동의.includes(`SNS`)}
                                  value={`SNS`}
                                  />
                                  <span>SNS</span>
                                </label>
                                <label>
                                  ...
                                  id='chk6'
                                  onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                  checked={this.state.이용약관동의.includes(`이메일`)}
                                  value={`이메일`}
                                  />
                                  <span>이메일</span>
                                </label>
                          ...
                          <dd>
                            <label>
                                ...
                                id='chk7'
                                onChange={(e)=>this.onChangeCheckEvent(e.target.checked, e.target.value)}
                                checked={this.state.이용약관동의.includes(`본인은 만 14세 이상입니다. (필수)`)}
              ...
              <div className='content-box'>
                <ul>
                  <li>
                    <h2>알림메시지</h2>
                    <span>
                      <a href='#!' onClick={this.onClickModalClose} title='close'><img src='./img/ico_close_999_32x32_x2.png' alt='close' /></a>
                    </span>
                  </li>
                  <li>
